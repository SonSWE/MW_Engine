...
{
    public sealed class BOAuthClient : IBOAuthClient, IDisposable
    {
        ...
        public BOAuthClient(string serverHost)
        {
            Logger.log.Info($"[BOAuthClient] Khoi tao serverHost=[{serverHost}]");
        ...
        private async Task WriteLogProcessTimeThread(CancellationToken cancellationToken)
        {
            while (await _writeLogProcessTimeTimer.WaitForNextTickAsync(cancellationToken))
            {
                ...
                if (processTimeInfos != null && processTimeInfos.Count > 0)
                {
                    ...
                    {
                        Logger.log.Info($"[{guid}] [Log TPS BOAuthClient] methodName=[{item.MethodName}] numberRequest=[{item.NumberRequest}] maxProcessTime=[{item.MaxProcessTime}(ms)] minProcessTime=[{item.MinProcessTime}(ms)] avgProcessTime=[{Math.Round(item.TotalProcessTime / item.NumberRequest, 2)}(ms)]");
                    }
                }
            }
        }
        ...
        public async Task<bool> ValidateFunction(string userId, string functionId, string action, string checkMode = "")
        {
            ...
            Logger.log.Info($"[{requestId}] [BOAuthClient.ValidateFunction] dia chi [{_httpClient?.BaseAddress}] tham so userId=[{userId}] userPid=[{functionId}] action=[{action}]");
            ...
            Logger.log.Info($"[{requestId}] [BOAuthClient.ValidateFunction] responseCode=[{(int)response.StatusCode}-{response.StatusCode}]");
        ...
        endFunc:
            ...
            Logger.log.Info($"[{requestId}] [BOAuthClient.ValidateFunction] thoi gian xu ly {processTime}(ms)");
            RecordProcessTime("BOAuthClient.ValidateFunction", processTime);
        ...
        public async Task<Tuple<HttpStatusCode, LoggedUser>> ValidateTokenAndFunction(string token, string functionId, string action, bool checkFunction, string checkMode = "")
        {
            var requestId = Utils.GenGuidStringN();
            var requestTime = DateTime.Now;
            Logger.log.Info($"[{requestId}] [BOAuthClient.ValidateTokenAndFunction] dia chi [{_httpClient?.BaseAddress}] tham so userPid=[{functionId}] action=[{action} checkFunction=[{checkFunction}]");
            ...
            Logger.log.Info($"[{requestId}] [BOAuthClient.ValidateTokenAndFunction] responseCode=[{(int)response.StatusCode}-{response.StatusCode}]");
            ...
            if (response.StatusCode == HttpStatusCode.OK)
            {
                ...
                Logger.log.Info($"[{requestId}] [BOAuthClient.ValidateTokenAndFunction] Authenticated userId={user?.UserName}; name={user?.Name}; branchId={user?.BranchId};");
            }


            var processTime = ConstLog.GetProcessingMilliseconds(requestTime);
            Logger.log.Info($"[{requestId}] [BOAuthClient.ValidateTokenAndFunction] thoi gian xu ly {processTime}(ms)");
            RecordProcessTime("BOAuthClient.ValidateTokenAndFunction", processTime);
        ...
        public async Task<Tuple<HttpStatusCode, LoggedUser>> ValidateTokenAndUrl(string token, string requestUrl, bool isCheckUrl = true)
        {
            var requestId = Utils.GenGuidStringN();
            var requestTime = DateTime.Now;
            Logger.log.Info($"[{requestId}] [BOAuthClient.ValidateTokenAndUrl] dia chi [{_httpClient?.BaseAddress}] tham so requestUrl=[{requestUrl}] isCheckUrl=[{isCheckUrl}]");
            ...
            Logger.log.Info($"[{requestId}] [BOAuthClient.ValidateTokenAndUrl] responseCode=[{(int)response.StatusCode}-{response.StatusCode}]");
            ...
            if (response.StatusCode == HttpStatusCode.OK)
            {
                ...
                Logger.log.Info($"[{requestId}] [BOAuthClient.ValidateTokenAndUrl] Authenticated userId={user?.UserName}; name={user?.Name}; branchId={user?.BranchId}");
            }

            var processTime = ConstLog.GetProcessingMilliseconds(requestTime);
            Logger.log.Info($"[{requestId}] [BOAuthClient.ValidateTokenAndUrl] thoi gian xu ly {processTime}(ms)");
            RecordProcessTime("BOAuthClient.ValidateTokenAndUrl", processTime);
